...
Текст автоматически переносится, чтобы максимально сбалансировать количество символов на каждой строке

```css
.element { text-wrap: balance; }
```

⚠️ Подсчет символов и балансировка строк — ресурсоемкий процесс, поэтому поддерживается только для коротких текстов (до 6 строк в Chromium и до 10 строк в Firefox).
